...
import random
import string
from HomeRoot import HomeRoot
from HomeStu import HomeStu
from UserID import *
...
def Login():
    ...
    def btnSet():
        gene_code()
        photo = pil.Image.open('../image/gene_code.png')
    ...
    # 初始化控件
    gene_code()
    f_time = tk.Frame(login)
    f_time.pack()
    f_username = tk.Frame(login)
    f_username.pack()
    f_password = tk.Frame(login)
    f_password.pack()
    f_verify = tk.Frame(login)
    f_verify.pack()
    f_log = tk.Frame(login)
    f_log.pack()
...
def gene_code():
    ...
    image_path = os.path.join(lib_path, 'gene_code.png')
